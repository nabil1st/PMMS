...
public class IncomeSourceBuilder {
     ...
     */
    public static IncomeSourceBean createIncomeSourceBean(IncomeSource account) throws EntityException {
        IncomeSourceBean accountBean = new IncomeSourceBean();

        try {
            BeanUtils.copyProperties(accountBean, account);
        } catch (Exception e) {
            throw new EntityException("Could not build IncomeSourceBean " + e.toString(), e);
        }

        return accountBean;
    }

    public static IncomeSource createIncomeSource(IncomeSourceBean accountBean) throws EntityException {
        IncomeSource account = new IncomeSource();

        try {
            BeanUtils.copyProperties(account, accountBean);
        } catch (Exception e) {
            throw new EntityException("Could not build IncomeSource " + e.toString(), e);
        }
