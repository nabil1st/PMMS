...
public class BorrowerBuilder {
     ...
     */
    public static BorrowerBean createBorrowerBean(Borrower account) throws EntityException {
        BorrowerBean accountBean = new BorrowerBean();

        try {
            BeanUtils.copyProperties(accountBean, account);
        } catch (Exception e) {
            throw new EntityException("Could not build BorrowerBean " + e.toString(), e);
        }

        return accountBean;
    }

    public static Borrower createBorrower(BorrowerBean accountBean) throws EntityException {
        Borrower account = new Borrower();

        try {
            BeanUtils.copyProperties(account, accountBean);
        } catch (Exception e) {
            throw new EntityException("Could not build Borrower " + e.toString(), e);
        }
